...
struct next_job *get_min_queue_time(struct event_list events, int server_occupation, int index)
{
    // GetMinCompletion
    ...
    {
    case 1:
        completionTimes = events.completionTimes_web_server;
        break;
    case 2:
        completionTimes = events.completion_times_spike_server;
        break;
    default:
        printf("Error: the value of param index must be a number beteween 1 and 2!\n");
        fflush(stdout);
        exit(-8);
    }
...
double get_minimum_time(struct event_list events, struct states *state)
{
    double times[DIM];
    ...
    struct next_job *next_queue_time_web_server = get_min_queue_time(events, state[0].server_occupation, 1);
    struct next_job *next_queue_time_spike_server = get_min_queue_time(events, state[1].server_occupation, 2);

    double min_service_web_server = next_queue_time_web_server->completionTime;
    double min_service_spike_server = next_queue_time_spike_server->completionTime;

    times[0] = min_service_web_server;
    times[1] = min_service_spike_server;
    times[2] = events.user_arrival_to_web_server.user_arrival_time;
    times[3] = events.user_arrival_to_spike_server.user_arrival_time;
